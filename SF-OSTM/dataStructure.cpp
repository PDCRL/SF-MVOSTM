								  ...
								  G_node** G_currs)
{
    ...
    if(G_currs[1]->G_key == L_key)
    {
        ...
        G_currs[1]->G_vl->at(0)->G_rvl->push_back(tranx);
        ...
        if(G_currs[1]->G_vl->at(0)->G_val == -1)
        {
			L_opn_status = FAIL;
            L_val = NULL;
        } else {
            L_opn_status = OK;
            L_val = &(G_currs[1]->G_vl->at(0)->G_val);
        }
    }
 ...
 * in sorted order.
 */
void HashMap::insertVersion(int L_cts, int L_val, G_node* key_Node)
{
    version *newVersion = new version();
    newVersion->G_ts = L_cts;
