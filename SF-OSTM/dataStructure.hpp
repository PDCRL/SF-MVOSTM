...
public:
    ...
    OPN_STATUS commonLuNDel(vector<G_node*> *lockedNodes, L_txlog *tranx, int L_bucket_id, int L_key, int* L_val, G_node** G_preds, G_node** G_currs);
    
    /*Method to add a version in the appropriate Key_node version list in sorted order.*/
    void insertVersion(int L_cts, int L_val, G_node* key_Node);
